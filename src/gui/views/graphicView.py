...
"""
Graphics view :
    - Author : Guillaume Touchet, David-Henri Arnaud
    - Display three drop down menu buttons that allows to select the type of graphic, the main category of statistics
      he wants to display, and the sub-category of statistics
    - Once every value has been selected, display the statistics graphic and the download button
    - Clicking on the download button downloads the graphic as a PNG, placing the file in the "/download" folder
"""
...
def displayGraph(self):
    ...
    self.graphic = plt.figure(figsize=(7, 5), dpi=100)

    if self.graphicType == "Pie chart":
        plt.pie(percents, labels=stats, autopct='%1.1f%%')
        plt.axis('equal')

    elif self.graphicType == "Bars":
        ...
        self.graphic.add_subplot(111).plot(stats, percents)
        plt.ylabel(dataToDisplay[1])
        plt.xlabel(dataToDisplay[2])
        plt.subplots_adjust(bottom=0.45)
        plt.xticks(rotation=45, ha='right')
...
def displayDropDownMenuGraphicTypeChoice(self):
    graphicTypeList = [
        "Pie chart",
        "Bars",
        "Linear",
    ...
    dropDownMenu = tk.OptionMenu(self, choice, *graphicTypeList)
    dropDownMenu.config(width=10, font=('Helvetica', 12))
    dropDownMenu.place(x=65, y=15, anchor=CENTER)

    def callback(*args):
        self.graphicType = format(choice.get())
        if self.mainCategory is not None and self.subCategory is not None:
            displayGraph(self)
...
def displayDropDownMenuMainCategoryChoice(self):
    ...
    def callback(*args):
        ...
        if self.graphicType is not None and self.subCategory is not None:
            displayGraph(self)
            displayDownloadFileButton(self)
...
def displayDropDownMenuSubCategoryChoice(self):
    ...
    def callback(*args):
        ...
        if self.graphicType is not None and self.mainCategory is not None:
            displayGraph(self)
            displayDownloadFileButton(self)
    menu.trace("w", callback)
